...
- 书籍续写（长书）：
  - 续写页 → 书籍续写
  - 上传书籍源（本地落盘）
  - 章节分块（可手动微调并保存；支持“标题藏在正文里/重复标题去噪”）
...
- 图谱 → 书籍结构图：
  ...
  - 章节关系（非线性边，LLM）：在书籍结构图内点击「生成章节关系（LLM）」会产出 `book_relations`，用于展示因果/伏笔-照应/人物弧光/主题深化/结构勾连/悬念等跨章关系（可开关、阈值过滤、显示标签）。
  - 人物关系图（LLM）：在书籍结构图页签内可切换到「人物关系图」，点击「生成人物关系（LLM）」会产出 `book_characters`，以人物卡形式展示角色信息与人物关系边（点击人物可查看详情）。
