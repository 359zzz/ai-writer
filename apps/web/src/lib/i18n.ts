...
const ZH: Record<I18nKey, string> = {
  ...
  import_to_kb: "导入到知识库",
  agents: "Agent 协作",
  agents_desc: "可视化多智能体执行轨迹（时间线 + 图谱）。",
  timeline: "时间线",
  graph: "图谱",
  events: "事件数",
  no_events: "暂无运行事件。请先在写作页运行一次流程。",
  execution_flow: "执行流",
  compressed_view: "图谱为聚合视图：按 Agent 汇总 tool/artifact，并显示产物类型计数。",
  settings: "设置",
...
const EN: Record<I18nKey, string> = {
  ...
  agents_desc: "Visualize multi-agent traces (timeline + graph).",
  timeline: "Timeline",
  graph: "Graph",
  events: "Events",
  ...
  compressed_view:
    "Aggregated graph: groups tool/artifact events per agent and counts artifact types.",
  settings: "Settings",
